...
local title = "Report System"
local fields = "Commands , System"
local footer = "Ann - Oliver One , '  ' "..formattedTime..""
...
addEventHandler("onResourceStart", getResourceRootElement(getThisResource()), function()
		...
		end, 300000, 0) -- Checks this every 5 minutes.
	end)
...
function handleReport(reportedPlayer, reportedReason, reportType)
	...
	local playerAcc = getElementData(source, "account:username")
		...
		if string.find(SUPPORTER, usergroup) then -- Supporters
			...
			for key, value in ipairs(GMs) do
				...
				if (gmDuty == 1) then
					...
					outputChatBox("Info - ID: ( " .. playerID .. " ) - Name: ( " .. tostring(getPlayerName(source) ) .. " - Account: ( " .. playerAcc .. " )", value, 200, 240, 120)
					local webhook = "https://discord.com/api/webhooks/992061514586538024/4dxF6arWXivgo52PMzYv-zpjHju6l8xpovn0NwRtHq74mfGHc0bLH3oAAO_p66YEoDrj"
					local description = "- Player id ["..playerID.. "]\n- Player Name [".. getPlayerName(source) .."]\n- Question [" .. reportedReason .. "]"
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					local description = "- Admin has accept report id [#"..id.. "]\n- Admin Rank [".. adminTitle .."]\n- Admin Name [" .. adminName .. "]\n- Player Name [" .. getPlayerName(reportingPlayer) .. "]"
					triggerEvent('Eminem:Logs', root, title, fields, description, footer, webhook)
					triggerClientEvent ( reportingPlayer, "playNudgeSound", reportingPlayer)
		...
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				if staff then
					...
					local webhook = "https://discord.com/api/webhooks/992507674669355030/eeNUeA0tKPmxICAHFCBZ3vb90URpm321eS8RVw9b5ruEFYUC_7g5Zf1BapHVEUsrXGtx"
					local description = "- Admin Name [".. adminName .."]\n- Admin Rank [" .. adminTitle .. "]\n- Closed Report Id [#" .. id .. "]"
					triggerEvent('Eminem:Logs', root, title, fields, description, footer, webhook)
											alreadyTold[value] = true
										end
									end
...
function saveReportCount()
	...
	if tonumber(adminreports) then
		mysql:query_free("UPDATE `accounts` SET `adminreports`='"..adminreports.."' WHERE `id` = " .. mysql:escape_string(getElementData( source, "account:id" )) )
	end

	local adminreports_saved = getElementData(source, "adminreports_saved")
	if tonumber(adminreports_saved) then
		mysql:query_free("UPDATE `accounts` SET `adminreports_saved`='"..adminreports_saved.."' WHERE `id` = " .. mysql:escape_string(getElementData( source, "account:id" )) )
	end
end
